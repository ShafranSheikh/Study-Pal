...
struct TimerView: View {
    ...
    var availableTasks: [StudyTask] {
        taskViewModel.tasks.filter { $0.status == "In Progress" }
    }
                    ...
                    VStack(spacing: 25) {
                        ...
                        VStack(alignment: .leading, spacing: 10) {
                            if timerViewModel.selectedTaskId.isEmpty {
                                HStack(spacing: 6) {
                                    ...
                                    Text("Choose an active task to start")
                                        .font(.caption.bold())
                                        .foregroundColor(.orange)
                                }
                                .padding(.leading, 20)
                            ...
                            Menu {
                                ...
                                HStack {
                                    Image(systemName: "list.bullet.clipboard")
                                        .foregroundColor(timerViewModel.selectedTaskId.isEmpty ? .gray : .blue)
                                    
                                    Text(timerViewModel.selectedTaskId.isEmpty ? "Select a task" : (availableTasks.first(where: { $0.id == timerViewModel.selectedTaskId })?.title ?? "Unknown Task"))
                                        .foregroundColor(timerViewModel.selectedTaskId.isEmpty ? .gray : .primary)
                                        .fontWeight(timerViewModel.selectedTaskId.isEmpty ? .regular : .semibold)
                                ...
                                .overlay(
                                    RoundedRectangle(cornerRadius: 15)
                                        .stroke(timerViewModel.selectedTaskId.isEmpty ? Color.orange.opacity(0.5) : Color.blue.opacity(0.3), lineWidth: 2)
                                )
                                .padding(.horizontal)
                            }
                            
                            if availableTasks.isEmpty && timerViewModel.selectedTaskId.isEmpty {
                                Text("No tasks are currently 'In Progress'.")
                                    .font(.caption)
                        ...
                        VStack(spacing: 35) {
                            Text(timerViewModel.timeString)
                                ...
                                .foregroundColor(timerViewModel.selectedTaskId.isEmpty ? .gray.opacity(0.5) : .primary)
                            ...
                            VStack(spacing: 20) {
                                ...
                                .opacity(timerViewModel.selectedTaskId.isEmpty ? 0.3 : 1.0)
                                
                                if timerViewModel.selectedTaskId.isEmpty {
                                    Text("Select a task above to enable controls")
                                        .font(.caption.italic())
                                        .foregroundColor(.secondary)
                                }
                            }
